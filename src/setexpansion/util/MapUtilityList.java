...
public class MapUtilityList
{
...

	{
		...
		TermListMap = new HashMap();
		file = new File(currentPath + "TermList.txt");
		try
		{
			LineIterator iterator = FileUtils.lineIterator(file, "UTF-8");
			while (iterator.hasNext())
			{
				String line = iterator.nextLine();
				String value[] = line.split("\t\t");
				TermListMap.put(value[0], value[1]);
	...
	public static double getSimilarityScore(String term1, String term2)
	{
		...
		String value1[] = TermListMap.get(term1).split("\t");
		String value2[] = TermListMap.get(term2).split("\t");
