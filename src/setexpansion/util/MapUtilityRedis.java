...
public class MapUtilityRedis
{
	...
	public static String currentPath = "D:\\CodePath\\dataset\\SyntheticData\\SEISA";

	{
		...
		/*
		 * termPairCountMap = new HashMap(); file = new File(currentPath + "smallTermPairCount.txt"); try { LineIterator iterator = FileUtils.lineIterator(file, "UTF-8"); while (iterator.hasNext()) {
		 * String line = iterator.nextLine(); String value[] = line.split("\t\t"); termPairCountMap.put(value[0], Integer.parseInt(value[1]));
		 * 
		 * // yu line = null; value = null; } System.out.println("Finished loading the term pair count"); } catch (IOException e) { // TODO Auto-generated catch block e.printStackTrace(); }
		 */
		//
	...
	public static double getSimilarityScore(String term1, String term2)
	{
		...
		if (jedis.exists(termToSearch).equals(1))
		{
			intersection = Integer.parseInt(jedis.get(termToSearch));
		} else
		{
